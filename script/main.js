...
function updateYesButtonSize() {
  ...
  const buttonSize = noCount++ * 20 + 60;
  yesButton.style.fontSize = `${buttonSize}px`;
  document.getElementById("noButton").innerText = getNoButtonText();
}
// Function to start animations based on user choice (yes or no)
function startAnimations(isYes) {
  const yesButton = document.getElementById("yesButton");
  const noButton = document.getElementById("noButton");
  const yesNoButtons = document.querySelector(".yes-no-buttons");
  const container = document.querySelector(".container");

  // Hide Yes and No buttons
  yesNoButtons.style.display = "none";

  // Display the container with animations
  container.style.display = "block";

  // If user clicked Yes, trigger animations
  if (isYes) {
    let elementToRemove = document.getElementById("removable");
